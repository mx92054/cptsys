...
paramDef	ParamsDef[] = {
	...
	{0,	5,	10,	0},		{0,	5,	11,	0},		{0,	5,	12,	0},		{0,	5,	13,	0},		{0,	5,	14,	0},			/*1000 - 1009*/
	/* 1#姿态运行     2#姿态运行        1#贯入额定电流    1#贯入动作时间    1#贯入速断倍数  */
	{0,	5,	60,0},		{0,	8,	61,	0},		{0,	5,	62,	0},		{0,	5,	63,	0},		{0,	5,	64,	0},	
	/*2#贯入额定电流  2#贯入动作时间    2#贯入速断倍数    油泵额定电流      油泵动作时间 */
	{0,	5,	65,	0},		{0,	5,	66,	0},		{0,	5,	67,	0},		{0,	5,	68,	0},		{0,	5,	69,	0},			/*1010 - 1019*/
	/*油泵速断倍数    1#姿态过流设定    2#姿态过流设定    姿态手自动         1#姿态速断设定*/
	{0,	5,	70,	0},		{0,	5,	71,	0},		{0,	5,	72,	0},		{0,	5,	73,	0},		{0,	5,	74,	0},	
	/*1#姿态方向设定  2#姿态速断设定    2#姿态方向设定     姿态设定开关     PITCH */
	{0,	5,	75,	0},		{0,	5,	76,	0},		{0,	5,	77,	0},		{0,	5,	78,	0},		{0,	5,	79,	0},		/*1020 - 1029*/
	/*YAW             姿态P							姿态I							姿态D             姿态输出限幅*/
	{0,	5,	80,	0},		{0,	5,	81,	0},		{0,	5,	82,	0},		{0,	5,	83,	0},		{0,	5,	84,	0},	
	
	{0,	2,	0,	1},		{0,	2,	1,	1},		{0,	3,0,	1},			{0,	3,	1,	1},		{0,	4,	0,	1},			/*1030 - 1039*/
	{0,	4,	1,	1},		{0,	13,	0,	0},		{0,	13,1,	0},			{0,	13,	2,	0},		{0,	13,	3,	0},		
	
	{0,	13,	4,	0},		{0,	13,	5,	0},		{0,	13,	6,	0},		{0,	13,	7,	0},		{0,	 9,	0,	0},			/*1040 - 1049*/
	{0,	9,	1,	0},		{0,	9,	2,	0},		{0,	9,	3,	0},		{0,	9,	4,	0},		{0,	9,	5,	0},		

	{0,	9,	6,	0},		{0,	9,	7,	0},		{0,	10,	0,	0},		{0,	10,	1,	0},		{0,	10,	2,	0},			/*1050 - 1059*/
	{0,	10,	3,	0},		{0,	10,	4,	0},		{0,	10,	5,	0},		{0,	10,	6,	0},		{0,	10,	7,	0},	
	/*剪切装置命令设置*/
	{0,	14,	100,	0},		{0,	14,	101,	0},		{0,	14,	102,	0},		{0,	14,	103,	0},		{0,	14,	104,	0},			/*1060 - 1069*/
	{0,	14,	105,	0},		{0,	14,	106,	0},		{0,	14,	107,	0},		{0,	14,	108,	0},		{0,	14,	109,	0},	
		
	{0,	14,	110,	0},		{0,	14,	111,	0},		{0,	14,	112,	0},		{0,	14,	113,	0},		{0,	14,	114,	0},			/*1070 - 1079*/
	{0,	14,	115,	0},		{0,	14,	116,	0},		{0,	14,	117,	0},		{0,	14,	118,	0},		{0,	14,	119,	0}	
};
...
void 	OrganizeData(void)
{
	...
	/* 电机控制板1*/
	for(i = 0 ; i < 3 ; i++)	usInputReg[i] = usHoldReg[240 + i] ;
	usInputReg[3] = 0 ;
	if ( usHoldReg[252] == 1 )	usInputReg[3] = 1 ;
	if ( usHoldReg[253] == 1 )	usInputReg[3] = 2 ;
	usInputReg[4] = 0 ;
	if ( usHoldReg[248] == 1 )	usInputReg[4] = 1 ;
	if ( usHoldReg[248] == 1 )	usInputReg[4] += 2 ;

	/* 电机控制板2*/
	for(i = 0 ; i < 3 ; i++)	usInputReg[5 + i] = usHoldReg[260 + i] ;
	usInputReg[8] = 0 ;
	if ( usHoldReg[272] == 1 )	usInputReg[8] = 1 ;
	if ( usHoldReg[273] == 1 )	usInputReg[8] = 2 ;
	usInputReg[9] = 0 ;
	if ( usHoldReg[268] == 1 )	usInputReg[9] = 1 ;
	if ( usHoldReg[268] == 1 )	usInputReg[9] += 2 ;

	/* 电机控制板3*/
	for(i = 0 ; i < 3 ; i++)	usInputReg[10 + i] = usHoldReg[280 + i] ;
	usInputReg[13] = 0 ;
	if ( usHoldReg[292] == 1 )	usInputReg[13] = 1 ;
	if ( usHoldReg[293] == 1 )	usInputReg[13] = 2 ;
	usInputReg[14] = 0 ;
	if ( usHoldReg[288] == 1 )	usInputReg[14] = 1 ;
	if ( usHoldReg[288] == 1 )	usInputReg[14] += 2 ;	
}	
